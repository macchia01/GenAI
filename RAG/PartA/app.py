...
class AI():
	def __init__(self):
		db = chromadb.PersistentClient(path="RAG\PartA\chroma")
	...
	def respond(self, lst_messages, model="llama3.2", use_knowledge=False):
		...
		if use_knowledge:
			...
			prompt = "Give the most accurate answer using only the folling information: \n"+context+". If you don't know the answer from the context, do not answer from your knowledge."
...
if txt := st.chat_input():
    ...
    app["full_response"] = ""
    st.chat_message("assistant", avatar="👾").write_stream( ai.respond(app["messages"], use_knowledge=False) )
